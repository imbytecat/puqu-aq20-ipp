// Package ipp exposes USB label printers through direct IPP operations.
package ipp
...
const printerPage = `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>PUQU IPP Bridge</title><style>body{font:16px system-ui;max-width:40rem;margin:4rem auto;padding:0 1rem;color:#111827}img{width:5rem}code{background:#eef2ff;padding:.2rem .4rem;border-radius:.25rem}</style><img src="/icon.svg" alt=""><h1>PUQU IPP Bridge</h1><p>Driverless network printer backed by direct USB.</p><p>Configure this bridge from its local administration interface.</p>`
